...
const (
	...
	grayFg            = (aurora.Color(232 + 12)) << 16
)

var bufferpool = buffer.NewPool()
var levelToColor map[zapcore.Level]aurora.Color

func init() {
	levelToColor = make(map[zapcore.Level]aurora.Color)
	levelToColor[zap.DebugLevel] = aurora.MagentaFg
	levelToColor[zap.InfoLevel] = aurora.WhiteFg
	levelToColor[zap.WarnLevel] = aurora.YellowFg
	levelToColor[zap.ErrorLevel] = aurora.RedFg
	levelToColor[zap.DPanicLevel] = aurora.RedFg
	levelToColor[zap.PanicLevel] = aurora.RedFg
	levelToColor[zap.FatalLevel] = aurora.RedFg
}
...
func (c PrettyConsoleEncoder) colorString(color, s string) (out string) {
...
func (c PrettyConsoleEncoder) EncodeEntry(ent zapcore.Entry, fields []zapcore.Field) (*buffer.Buffer, error) {
	line := bufferpool.Get()
	lineColor := levelColor(ent.Level)

	if c.showTime {
		line.AppendString(c.colorString(grayFg.Nos(true), ent.Time.Format("2006-01-02 15:04:05")+" "))
	}
	...
	if c.showLoggerName {
		...
		line.AppendString(c.colorString(aurora.BlueFg.Nos(true), "("+loggerName+")"+" "+ent.Level.CapitalString()+" "))
	}
	...
	if showCaller && ent.LoggerName != "box" {
		...
		line.AppendString(c.colorString(aurora.BlueFg.Nos(true), " ("+callerPath+")"))
	}
...
func (c PrettyConsoleEncoder) writeJSONFields(line *buffer.Buffer, extra []zapcore.Field) {
	context := c.Clone().(*PrettyConsoleEncoder)
...
func (enc *jsonEncoder) AppendTimeLayout(time time.Time, layout string) {
	enc.buf.AppendByte('"')
	enc.buf.AppendString(time.Format(layout))
	enc.buf.AppendByte('"')
}
